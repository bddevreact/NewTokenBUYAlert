...
class AdvancedTokenMonitor:
    def __init__(self, wallet_address: str, bot_token: str, chat_id: str, rpc_url: str = "https://api.mainnet-beta.solana.com"):
        ...
        self.METAPLEX_PROGRAM_ID = "metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s"
        ...
        try:
            ...
            for i, instruction in enumerate(instructions):
                ...
                if program_id == "spl-token" and instruction_type in [
                    ...
                    print(f"✅ Found {instruction_type} instruction!")
                    return True
            ...
            for inner_group in inner_instructions:
                for inner_instruction in inner_group.get('instructions', []):
                    ...
                    if program_id == "spl-token" and instruction_type in [
                        ...
                        print(f"✅ Found inner {instruction_type} instruction!")
                        return True
    ...
    def get_token_metadata_from_metaplex(self, mint_address: str) -> Dict:
        """Get token metadata from Metaplex metadata PDA"""
        try:
            # Metaplex PDA derivation: seeds = ["metadata", METAPLEX_PROGRAM_ID, mint]
            # This is a simplified approach - in production you'd use proper PDA derivation
            return {
                'name': 'Unknown Token',
                'symbol': 'UNKNOWN',
                'decimals': 9
            ...
            logger.error(f"Error getting Metaplex metadata: {e}")
            return {
                'name': 'Unknown Token',
                'symbol': 'UNKNOWN',
                'decimals': 9
    ...
    def get_token_metadata(self, mint_address: str) -> Dict:
        ...
        try:
            ...
            return self.get_token_metadata_from_metaplex(mint_address)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'result' in data and data['result']:
                    ...
                    oldest_signature = oldest_tx['signature']
                    
                    # Get transaction details for the oldest signature
                    oldest_tx_details = self.get_transaction_details(oldest_signature)
                    if oldest_tx_details:
                        first_tx_time = oldest_tx_details.get('blockTime', 0)
...
*Time:* {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"""
    ...
    def monitor_wallet(self, check_interval: int = 30):
        ...
        logger.info(f"Starting advanced token monitoring for: {self.wallet_address}")
        logger.info(f"Check interval: {check_interval} seconds")
        print(f"🔍 Advanced monitoring for NEW LAUNCHED TOKENS: {self.wallet_address}")
...
def main():
    ...
    monitor = AdvancedTokenMonitor(WALLET_ADDRESS, TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, RPC_URL)
...
if __name__ == "__main__":
    print("🤖 Advanced Solana New Token Launcher Monitor Bot")
    print("=" * 60)
    print("Using complete detection logic with jsonParsed encoding!")
    print("=" * 60)
