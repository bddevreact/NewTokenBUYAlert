...
class SolanaWalletMonitor:
    ...
    def is_new_token_created(self, transaction: Dict) -> bool:
        ...
        try:
            ...
            # ALL POSSIBLE TOKEN CREATION INSTRUCTIONS
            token_creation_types = [
                "createAccount", 
                "createTokenAccount",
                "initializeAccount",
                "initializeAccount2",
            ...
            for i, instruction in enumerate(instructions):
                ...
                # Check for ANY token creation instruction
                if program_id == "spl-token" and instruction_type in token_creation_types:
                    print(f"✅ Found token creation: {instruction_type}")
                    return True
                ...
                # Check for any program that might create tokens
                if program_id in ["spl-token", "spl-associated-token-account"] and instruction_type:
            ...
            for inner_group in inner_instructions:
                for inner_instruction in inner_group.get('instructions', []):
                    ...
                    # Check for ANY token creation instruction
                    if program_id == "spl-token" and instruction_type in token_creation_types:
                        print(f"✅ Found inner token creation: {instruction_type}")
                        return True
                    ...
                    # Check for any program that might create tokens
                    if program_id in ["spl-token", "spl-associated-token-account"] and instruction_type:
                        print(f"✅ Found inner token-related instruction: {program_id} -> {instruction_type}")
                        return True
...
*Time:* {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"""
    ...
    def monitor_wallet(self, check_interval: int = 5):
        """Main monitoring loop - ZERO MISSES, catch EVERYTHING"""
        ...
        while True:
            try:
                # Get recent transactions (maximum limit)
                transactions = self.get_recent_transactions(200)
                ...
                print(f"📈 Stats: {new_token_count} new tokens, {processed_count} already processed, {skipped_count} too old")
